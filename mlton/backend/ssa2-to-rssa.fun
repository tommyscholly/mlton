...
struct
  ...
  local open Ssa2
  in structure Base = Base
  end

  local open Runtime
  in structure GCField = GCField
  end
  ...
  struct
    ...
    val copyThread = fn () =>
      T { args = Vector.new2 (Type.gcState (), Type.thread ())
        ...
        , prototype = let open CType in (Vector.new2 (CPointer, CPointer), SOME CPointer) end
        , return = Type.thread ()
        , symbolScope = Private
    ...
    fun gcSequenceAllocate {return} =
      T { args = Vector.new4 (Type.gcState (), Type.csize (), Type.seqIndex (), Type.objptrHeader ())
        , convention = Cdecl
        , inline = false
        , kind = Kind.Runtime
            { bytesNeeded = NONE
        ...
        , prototype =
            (Vector.new4 (CType.gcState, CType.csize (), CType.seqIndex (), CType.objptrHeader ()), SOME CType.objptr)
        , return = return
        , symbolScope = Private
    ...
    fun gcSequenceCopy (dt, st) =
      T { args = Vector.new6 (Type.gcState (), dt, Type.seqIndex (), st, Type.seqIndex (), Type.seqIndex ())
        , convention = Cdecl
        , inline = false
        , kind = Kind.Runtime
            { bytesNeeded = NONE
        ...
        , prototype =
            ( Vector.new6
                (CType.gcState, CType.Objptr, CType.seqIndex (), CType.Objptr, CType.seqIndex (), CType.seqIndex ())
            , NONE
            )
        , return = Type.unit
        , symbolScope = Private
    ...
    val threadSwitchTo = fn () =>
      T { args = Vector.new3 (Type.gcState (), Type.thread (), Type.csize ())
        ...
        , prototype = (Vector.new3 (CType.gcState, CType.thread, CType.csize ()), NONE)
        , return = Type.unit
        , symbolScope = Private
    ...
    fun weakCanGet {arg} =
      T { args = Vector.new2 (Type.gcState (), arg)
        ...
        , prototype = (Vector.new2 (CType.gcState, CType.cpointer), SOME CType.bool)
        , return = Type.bool
        , symbolScope = Private
    ...
    fun weakNew {arg, return} =
      T { args = Vector.new3 (Type.gcState (), Type.objptrHeader (), arg)
        ...
        , prototype = (Vector.new3 (CType.gcState, CType.objptrHeader (), CType.cpointer), SOME (CType.cpointer))
        , return = return
        , symbolScope = Private
    ...
    fun share t =
      T { args = Vector.new2 (Type.gcState (), t)
        ...
        , kind = Kind.Runtime
            ...
            , ensuresBytesFree = NONE
            , mayGC = true
            , (* MLton.share works by tracing an object.
            ...
            , maySwitchThreadsTo = false
            , modifiesFrontier = true
            , (* actually, just readsFrontier *) readsStackTop = true
            , writesStackTop = true
            , readsRegionTop = false
            , writesRegionTop = false
            }
    ...
    fun size t =
      T { args = Vector.new2 (Type.gcState (), t)
        ...
        , kind = Kind.Runtime
            ...
            , ensuresBytesFree = NONE
            , mayGC = true
            , (* MLton.size works by tracing an object.
               * Make sure all the GC invariants are true,
               * because tracing might encounter the current
               * stack in the heap.
               *) maySwitchThreadsFrom = false
        ...
        , prototype = (Vector.new2 (CType.gcState, CType.cpointer), SOME (CType.csize ()))
        , return = Type.csize ()
        , symbolScope = Private
        , target = Direct "GC_size"
        }

    fun amAllocationProfiling () = Control.ProfileAlloc = ! Control.profile
    val intInfBinary = fn name =>
      CFunction.T
        { args = Vector.new4 (Type.gcState (), Type.intInf (), Type.intInf (), Type.csize ())
        , convention = Cdecl
        , inline = false
        , kind = CFunction.Kind.Runtime
            { bytesNeeded = SOME 3
            , ensuresBytesFree = NONE
            , mayGC = false
            , maySwitchThreadsFrom = false
        ...
        , prototype = (Vector.new4 (CType.gcState, CType.intInf, CType.intInf, CType.csize ()), SOME CType.intInf)
        , return = Type.intInf ()
        , symbolScope = Private
    ...
    val intInfCompare = fn name =>
      ...
      CFunction.T
        { args = Vector.new3 (Type.gcState (), Type.intInf (), Type.intInf ())
        , convention = Cdecl
        , inline = false
        , kind = CFunction.Kind.Runtime
            { bytesNeeded = NONE
        ...
        , prototype = (Vector.new3 (CType.gcState, CType.intInf, CType.intInf), SOME CType.compareRes)
    ...
    val intInfShift = fn name =>
      CFunction.T
        { args = Vector.new4 (Type.gcState (), Type.intInf (), Type.shiftArg, Type.csize ())
        , convention = Cdecl
        , inline = false
        , kind = CFunction.Kind.Runtime
            { bytesNeeded = SOME 3
        ...
        , prototype = (Vector.new4 (CType.gcState, CType.intInf, CType.shiftArg, CType.csize ()), SOME CType.intInf)
        , return = Type.intInf ()
        , symbolScope = Private
        , target = Direct (Prim.toString name)
        }
    val intInfToString = fn name =>
      (* CHECK; cint would be better? *)
      CFunction.T
        { args = Vector.new4 (Type.gcState (), Type.intInf (), Type.word WordSize.word32, Type.csize ())
        , convention = Cdecl
        , inline = false
        , kind = CFunction.Kind.Runtime
            { bytesNeeded = SOME 3
        ...
        , prototype = (Vector.new4 (CType.gcState, CType.intInf, CType.Int32, CType.csize ()), SOME CType.string)
        , return = Type.string ()
        , symbolScope = Private
        , target = Direct (Prim.toString name)
        }
    val intInfUnary = fn name =>
      CFunction.T
        { args = Vector.new3 (Type.gcState (), Type.intInf (), Type.csize ())
        , convention = Cdecl
        , inline = false
        , kind = CFunction.Kind.Runtime
            { bytesNeeded = SOME 2
        ...
        , prototype = (Vector.new3 (CType.gcState, CType.intInf, CType.csize ()), SOME CType.intInf)
        , return = Type.intInf ()
        , symbolScope = Private
        , target = Direct (Prim.toString name)
        }
  ...
  structure Prim =
  struct
    local structure CFunction' = CFunction
    ...
    fun cFunctionRaise (p: t) : CFunction.t =
      let
        ...
        fun coerce (t1, ct1, t2, ct2) =
          vanilla {args = Vector.new1 t1, name = name, prototype = (Vector.new1 ct1, SOME ct2), return = t2}
        local
          fun make n s =
            let
              val t = real s
              val ct = CType.real s
            in
              vanilla {args = Vector.new (n, t), name = name, prototype = (Vector.new (n, ct), SOME ct), return = t}
            end
        in val realBinary = make 2 val realTernary = make 3 val realUnary = make 1
        end
        fun realCompare s =
          let
            val t = real s
          in
            vanilla
              { args = Vector.new2 (t, t)
              , name = name
              , prototype = let val t = CType.real s in (Vector.new2 (t, t), SOME CType.bool) end
              , return = Type.bool
              }
          end
        local
        ...
        fun wordShift (s, sg) =
          ...
          in
            vanilla
              { args = Vector.new2 (t, Type.shiftArg)
              , name = name
      ...
      in
        ...
        | Real_castToWord (s1, s2) => coerce (real s1, realCType s1, word s2, wordCType (s2, {signed = false}))
        ...
        | Real_ldexp s =>
            ...
            in
              vanilla
                { args = Vector.new2 (t, Type.cint ())
                , name = name
                , prototype = (Vector.new2 (ct, CType.cint ()), SOME ct)
                , return = t
                }
            end
        ...
        | Real_rndToReal (s1, s2) => coerce (real s1, realCType s1, real s2, realCType s2)
        | Real_rndToWord (s1, s2, sg) => coerce (real s1, realCType s1, word s2, wordCType (s2, sg))
        ...
        | Word_castToReal (s1, s2) => coerce (word s1, wordCType (s1, {signed = false}), real s2, realCType s2)
        | Word_equal s => wordCompare (s, {signed = false})
        | Word_extdToWord (s1, s2, sg) =>
            coerce (word s1, wordCType (s1, sg), word s2, wordCType (s2, {signed = false}))
        ...
        | Word_rndToReal (s1, s2, sg) => coerce (word s1, wordCType (s1, sg), real s2, realCType s2)
  ...
  structure PackedRepresentation = PackedRepresentation (structure Rssa = Rssa structure Ssa2 = Ssa2)
  ...
  val cardSizeLog2: IntInf.t = 8
  (* must agree with CARD_SIZE_LOG2 in gc.c *)
  ...
  fun updateCard (addr: Operand.t) : Statement.t list =
    ...
    in
      [ PrimApp
          { args = (Vector.new2 (Operand.cast (addr, Type.bits (WordSize.bits sz)), Operand.word
  ...
  fun convertWordSize (ws: WordSize.t) : WordSize.t = WordSize.roundUpToPrim ws

  fun convertWordX (w: WordX.t) : WordX.t =
    WordX.resize (w, convertWordSize (WordX.size w))

  fun convert
    (program as S.Program.T {functions, globals, main, ...}, {codegenImplementsPrim: Rssa.Type.t Rssa.Prim.t -> bool}) :
    Rssa.Program.t =
    let
      ...
      val () = Vector.foreachi (objectTypes, fn (i, (opt, _)) => ObjptrTycon.setIndex (opt, i))
      val objectTypes = Vector.map (objectTypes, #2)
      val () = diagnostic ()

      val newObjectTypes = ref []
      local
        fun componentsHash cs = Prod.hash (cs, Type.hash)
        fun componentsEquals (cs1, cs2) = Prod.equals (cs1, cs2, Type.equals)
        val h = HashTable.new {hash = componentsHash, equals = componentsEquals}
      in
        fun allocRawOpt components =
          HashTable.lookupOrInsert (h, components, fn () =>
            let
              val rawComponents = Prod.map (components, fn ty =>
                if Type.isObjptr ty then Type.bits (Type.width ty) else ty)
              val rawTy = ObjectType.Sequence {components = rawComponents, hasIdentity = true}
              val rawOpt = ObjptrTycon.new ()
              val () = ObjptrTycon.setIndex (rawOpt, Vector.length objectTypes + HashTable.size h)
              val () = List.push (newObjectTypes, rawTy)
            in
              rawOpt
            end)
      end

      val {get = varInfo: Var.t -> {ty: S.Type.t}, set = setVarInfo, ...} = Property.getSetOnce
        (Var.plist, Property.initRaise ("varInfo", Var.layout))
      val setVarInfo = Trace.trace2 ("SsaToRssa.setVarInfo", Var.layout, S.Type.layout o #ty, Unit.layout) setVarInfo
      val varType = #ty o varInfo
      fun varOp (x: Var.t) : Operand.t =
        Var {var = x, ty = valOf (toRtype (varType x))}
      val varOp = Trace.trace ("SsaToRssa.varOp", Var.layout, Operand.layout) varOp
      fun varOps xs = Vector.map (xs, varOp)
      val extraBlocks = ref []
      fun newBlock {args, kind, statements: Statement.t vector, transfer: Transfer.t} : Label.t =
        let
          val l = Label.newNoname ()
          val _ = List.push (extraBlocks, Block.T
            {args = args, kind = kind, label = l, statements = statements, transfer = transfer})
        in
          l
        end
      val
        { get =
            labelInfo:
              (Label.t
               -> {args: (Var.t * S.Type.t) vector, cont: (Handler.t * Label.t) list ref, handler: Label.t option ref})
        , set = setLabelInfo
        , ...
        } = Property.getSetOnce (Label.plist, Property.initRaise ("label info", Label.layout))
      fun translateCase ({test: Var.t, cases: (Con.t, Label.t) S.Cases.t, default: Label.t option}) :
                 ...
                 (case S.Type.dest (varType test) of
                    S.Type.Datatype tycon =>
                      let
                        ...
                        val cases = Vector.map (cases, fn (con, dst) =>
                          { con = con
                          , dst = dst
                          , dstHasArg = Vector.fold (#args (labelInfo dst), false, fn ((_, ty), b) =>
                              b orelse isSome (toRtype ty))
                          })
                        val (ss, t, blocks) = genCase {cases = cases, default = default, test = test, tycon = tycon}
                        val () = extraBlocks := blocks @ !extraBlocks
                      in
                        (ss, t)
                      end
        ...
        | S.Cases.Word (s, cases) =>
            let
              val cases =
                QuickSort.sortVector (Vector.map (cases, fn (w, l) => (convertWordX w, l)), fn ((w, _), (w', _)) =>
                  WordX.le (w, w', {signed = false}))
            in
              ( []
              , Switch (Switch.T
                  {cases = cases, default = default, expect = NONE, size = convertWordSize s, test = varOp test})
              )
            end
      fun eta (l: Label.t, kind: Kind.t) : Label.t =
        let
          val {args, ...} = labelInfo l
          val args = Vector.keepAllMap (args, fn (x, t) => Option.map (toRtype t, fn t => (Var.new x, t)))
          val l' = Label.new l
          val _ = List.push (extraBlocks, Block.T
            { args = args
            , kind = kind
            ...
            , transfer = (Transfer.Goto {dst = l, args = Vector.map (args, fn (var, ty) => Var {var = var, ty = ty})})
            })
        in
          l'
        end
      ...
      val labelCont = Trace.trace2 ("SsaToRssa.labelCont", Label.layout, Handler.layout, Label.layout) labelCont
      fun vos (xs: Var.t vector) =
        Vector.keepAllMap (xs, fn x => Option.map (toRtype (varType x), fn _ => varOp x))
      ...
      val handlesSignals =
        S.Program.hasPrim (program, fn p =>
          ...
          | _ => false) orelse ! Control.forceHandlesSignals
      fun translateFormals v =
        Vector.keepAllMap (v, fn (x, t) => Option.map (toRtype t, fn t => (x, t)))
      fun translatePrim p =
        Prim.map (p, fn t =>
          case toRtype t of
            NONE => Type.unit
            ...
            let
              ...
              val return =
                ...
                | NonTail {cont, handler} =>
                    ...
                    in
                      Return.NonTail {cont = labelCont (cont, handler), handler = handler}
                    end
                | Tail => Return.Tail
            in
              ([], Transfer.Call {func = func, args = vos args, return = return})
            end
        | S.Transfer.Case r => translateCase r
        | S.Transfer.Goto {dst, args} => ([], Transfer.Goto {dst = dst, args = vos args})
               ...
               Prim.Thread_copyCurrent =>
                 let
                   ...
                   val l = newBlock
                     ...
                     , statements = Vector.new0 ()
                     , transfer = (Goto {args = Vector.new0 (), dst = return})
                     }
                 in
                   ( []
                   , Transfer.CCall {args = Vector.concat [Vector.new1 GCState, vos args], func = func, return = SOME l}
                   )
                 end
             | _ => Error.bug (concat ["SsaToRssa.translateTransfer: ", "strange Runtime prim: ", Prim.toString prim]))
      fun translateStatementsTransfer (statements, ss, transfer) =
        let
          fun loop (i, ss, t) : Statement.t vector * Transfer.t =
            if i < 0 then
              (Vector.fromList ss, t)
            else
              let
                fun none () =
                  loop (i - 1, ss, t)
              ...
              in
                    ...
                    (case toRtype (varType value) of
                     ...
                     | SOME t =>
                         let
                           ...
                           val ss = update
                             {base = baseOp, baseTy = varType (Base.object base), offset = offset, value = varOp value}
                           val ss =
                             if ! Control.markCards andalso Type.isObjptr t then updateCard (Base.object baseOp) @ ss
                             else ss
                         in
                           adds ss
                         end)
                | S.Statement.Bind {exp, ty, var} =>
                    let
                      fun split (args, kind, ss: Statement.t list, make: Label.t -> Statement.t list * Transfer.t) =
                        let
                          val l = newBlock {args = args, kind = kind, statements = Vector.fromList ss, transfer = t}
                          val (ss, t) = make l
                        in
                          loop (i - 1, ss, t)
                        end
                      fun maybeMove (f: Type.t -> Operand.t) =
                        case toRtype ty of
                          NONE => none ()
                        | SOME ty => add (Bind {dst = (valOf var, ty), pinned = false, src = f ty})
                             ...
                             Const.IntInf i =>
                               ...
                               in
                                 case Const.IntInfRep.fromIntInf i of
                                   Const.IntInfRep.Big v => doit (Const.WordVector v)
                                 | Const.IntInfRep.Small w => doit (Const.Word w)
                               end
                           | Const.Word w => move (Const (Const.Word (convertWordX w)))
                           | _ => move (Const c))
                      | S.Exp.Inject {variant, ...} => if isSome (toRtype ty) then move (varOp variant) else none ()
                          ...
                          (case toRtype ty of
                           ...
                           | SOME dstTy =>
                               let
                                 ...
                                 val _ =
                                   if Mode.equals (mode, Mode.Stack) then
                                     Error.warning (Layout.toString (List.layout Statement.layout obj))
                                   else
                                     ()
                               in
                                 adds (obj)
                      ...
                      | S.Exp.PrimApp {args, prim} =>
                          let
                            ...
                            fun ifIsWeakPointer (ty: S.Type.t, yes, no) =
                              case S.Type.dest ty of
                                S.Type.Weak ty =>
                                  (case toRtype ty of
                                     NONE => no ()
                                   | SOME t => if Type.isObjptr t then yes t else no ())
                              | _ => Error.bug "SsaToRssa.ifIsWeakPointer"
                            fun arrayOrVectorLength () =
                              move
                                (Offset {base = a 0, offset = Runtime.sequenceLengthOffset (), ty = Type.seqIndex ()})
                            fun subWord s =
                              let
                                val ty = Type.word s
                              in
                                move (SequenceOffset
                                  {base = a 0, index = a 1, offset = Bytes.zero, scale = Type.scale ty, ty = ty})
                              end
                            fun dst () =
                              case var of
                                SOME x => Option.map (toRtype (varType x), fn t => (x, t))
                              | NONE => NONE
                            fun primApp (prim, args) =
                              add (PrimApp {dst = dst (), prim = prim, args = args})
                            fun bumpAtomicState n =
                              ...
                              in
                                ( [ Statement.PrimApp
                                      { args = (Vector.new2 (atomicState, (Operand.word
                                          (WordX.fromInt (n, WordSize.word32)))))
                                      , dst = SOME (res, resTy)
                                      , prim = Prim.Word_add WordSize.word32
                                      }
                                  , Statement.Move {dst = atomicState, src = resOper}
                                  ]
                                , resOper
                                )
                              end
                            fun ccall {args: Operand.t vector, func: CFunction.t} =
                              let
                                val formals =
                                  case dst () of
                                    NONE => Vector.new0 ()
                              ...
                              in
                                split (formals, Kind.CReturn {func = func}, ss, fn l =>
                                  ([], Transfer.CCall {args = args, func = func, return = SOME l}))
                              end
                            fun simpleCCall (f: CFunction.t) =
                              ccall {args = vos args, func = f}
                            fun simpleCCallWithGCState (f: CFunction.t) =
                              ccall {args = Vector.concat [Vector.new1 GCState, vos args], func = f}
                            fun sequenceAlloc (numElts: Operand.t, opt) =
                              let
                                val result = valOf (toRtype ty)
                                val args = Vector.new4
                                  (GCState, Operand.zero (WordSize.csize ()), numElts, ObjptrTycon opt)
                                val func = CFunction.gcSequenceAllocate {return = result}
                              in
                            ...
                            fun codegenOrC (p: Prim.t, args) =
                              if codegenImplementsPrim p then
                                primApp (p, args)
                              else
                                (case Prim.cFunction p of
                                   NONE =>
                                     Error.bug
                                       (concat ["SsaToRssa.codegenOrC: ", "unimplemented prim:", Prim.toString p])
                          ...
                          in
                                  ...
                                  val allocRawOpt = fn () =>
                                    let
                                      ...
                                      val arrOpt =
                                        case Type.deObjptr result of
                                          NONE =>
                                            Error.bug "SsaToRssa.translateStatementsTransfer: PrimApp,Array_allocRaw"
                                        | SOME arrOpt => arrOpt
                                      val arrTy = Vector.sub (objectTypes, ObjptrTycon.index arrOpt)
                                      val arrComponents =
                                        case arrTy of
                            ...
                            | Prim.Array_copyArray =>
                                simpleCCallWithGCState (CFunction.gcSequenceCopy (Operand.ty (a 0), Operand.ty (a 2)))
                            ...
                            | Prim.Array_toArray =>
                                ...
                                in
                                  add2
                                    ...
                                    , Bind
                                        {dst = (valOf var, arrTy), pinned = false, src = Operand.cast (rawarr, arrTy)}
                                    )
                                end
                            | Prim.Array_toVector =>
                                let
                                ...
                                in
                                  add2
                                    ...
                                    , Bind
                                        {dst = (valOf var, vecTy), pinned = false, src = Operand.cast (sequence, vecTy)}
                                    )
                                end
                            | Prim.Array_uninit =>
                                let
                                  ...
                                  val eltTys =
                                    ...
                                    | SOME eltTys => eltTys
                                  val sss = Vector.toListKeepAllMapi (S.Prod.dest eltTys, fn (offset, {elt, ...}) =>
                                    case toRtype elt of
                                      NONE => NONE
                                    | SOME elt =>
                                        if not (Type.isObjptr elt) then
                                          NONE
                                        else
                                          (SOME o update)
                                            { base = Base.SequenceSub {index = index, sequence = sequence}
                                            , baseTy = sequenceTy
                                            , offset = offset
                                            , value = bogus elt
                                            })
                            ...
                            | Prim.Array_uninitIsNop =>
                                let
                                  ...
                                  val eltTys =
                                    case S.Type.deSequenceOpt sequenceTy of
                                      NONE =>
                                        Error.bug "SsaToRssa.translateStatementsTransfer: PrimApp,Array_uninitIsNop"
                                    | SOME eltTys => eltTys
                                  val isNop = Vector.forall (S.Prod.dest eltTys, fn {elt, ...} =>
                                    case toRtype elt of
                                      NONE => true
                            ...
                            | Prim.GC_collect =>
                                ccall
                                  { args = (Vector.new3 (GCState, Operand.zero (WordSize.csize ()), Operand.bool true))
                                  , func = (CFunction.gc {maySwitchThreads = handlesSignals})
                                  }
                            | Prim.GC_state => move GCState
                            | Prim.IntInf_add => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_andb => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_arshift => simpleCCallWithGCState (CFunction.intInfShift prim)
                            | Prim.IntInf_compare => simpleCCallWithGCState (CFunction.intInfCompare prim)
                            | Prim.IntInf_gcd => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_lshift => simpleCCallWithGCState (CFunction.intInfShift prim)
                            | Prim.IntInf_mul => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_neg => simpleCCallWithGCState (CFunction.intInfUnary prim)
                            | Prim.IntInf_notb => simpleCCallWithGCState (CFunction.intInfUnary prim)
                            | Prim.IntInf_orb => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_quot => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_rem => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_sub => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.IntInf_toString => simpleCCallWithGCState (CFunction.intInfToString prim)
                            | Prim.IntInf_toVector => cast ()
                            | Prim.IntInf_toWord => cast ()
                            | Prim.IntInf_xorb => simpleCCallWithGCState (CFunction.intInfBinary prim)
                            | Prim.MLton_bug =>
                                loop
                                  (i - 1, [], Transfer.CCall {args = vos args, func = CFunction.bug (), return = NONE})
                            | Prim.MLton_bogus =>
                                (case toRtype ty of
                                   NONE => none ()
                                 | SOME t => move (bogus t))
                                     ...
                                     let
                                       ...
                                       val (prim, args) =
                                         case Type.toCType t of
                                           CType.CPointer => (Prim.CPointer_equal, args)
                                         | CType.Objptr =>
                                             (wordEqual, Vector.map (args, fn arg => Operand.cast (arg, Type.word ws)))
                                         ...
                                         | _ =>
                                             Error.bug (concat
                                               [ "SsaToRssa.translateStatementsTransfer: PrimApp,MLton_eq,"
                                               , Layout.toString (Type.layout t)
                                               ])
                                     in
                                       codegenOrC (prim, args)
                                     end)
                            | Prim.MLton_halt => simpleCCallWithGCState (CFunction.halt ())
                            | Prim.MLton_installSignalHandler => none ()
                            | Prim.MLton_share =>
                                (case toRtype (varType (arg 0)) of
                                   NONE => none ()
                                 | SOME t =>
                                     if not (Type.isObjptr t) then none ()
                                     else simpleCCallWithGCState (CFunction.share (Operand.ty (a 0))))
                            | Prim.MLton_size =>
                                (case toRtype (varType (arg 0)) of
                                   NONE => move (Operand.word (WordX.zero (WordSize.csize ())))
                                 | SOME t =>
                                     if not (Type.isObjptr t) then move (Operand.word (WordX.zero (WordSize.csize ())))
                                     else simpleCCallWithGCState (CFunction.size (Operand.ty (a 0))))
                            | Prim.MLton_touch =>
                                let
                                  val a = arg 0
                                in
                                  if isSome (toRtype (varType a)) then primApp (prim, Vector.new1 (varOp a))
                                  else none ()
                                end
                                      ...
                                      in
                                        newBlock
                                          ...
                                          , statements =
                                              Vector.new2
                                                ( Statement.PrimApp
                                                    { args = (Vector.new2 (Runtime LimitPlusSlop, Operand.word
                                                        (WordX.fromBytes (Runtime.limitSlop, size))))
                                                    , dst = SOME (tmp, ty)
                                                    , prim = Prim.CPointer_sub
                                                    }
                                                , Statement.Move {dst = Runtime Limit, src = Var {ty = ty, var = tmp}}
                                                )
                                          , transfer = Transfer.Goto {args = Vector.new0 (), dst = continue}
                                          }
                                      end
                                    val (bumpAtomicStateStmts, _) = bumpAtomicState 1
                                  in
                                    ( bumpAtomicStateStmts
                                    , if handlesSignals then
                                        Transfer.ifBool
                                          (Runtime SignalIsPending, {falsee = continue, truee = signalIsPending ()})
                                      else
                                        Transfer.Goto {args = Vector.new0 (), dst = continue}
                                    )
                                  end)
                                ...
                                split (Vector.new0 (), Kind.Jump, ss, fn continue =>
                                  let
                                    ...
                                    val args = Vector.new3
                                      (GCState, Operand.zero (WordSize.csize ()), Operand.bool false)
                                    fun switchToHandler () =
                                      newBlock
                                        { args = Vector.new0 ()
                                        , kind = Kind.Jump
                                        ...
                                        , transfer = Transfer.CCall
                                            {args = args, func = func, return = SOME (returnFromHandler ())}
                                        }
                                    fun testSignalIsPending () =
                                      newBlock
                                        ...
                                        , transfer = Transfer.ifBool
                                            (Runtime SignalIsPending, {falsee = continue, truee = switchToHandler ()})
                                        }
                                    val (bumpAtomicStateStmts, bumpAtomicStateTmp) = bumpAtomicState ~1
                                  in
                                    ( bumpAtomicStateStmts
                                    , if handlesSignals then
                                        Transfer.ifZero
                                          (bumpAtomicStateTmp, {falsee = continue, truee = testSignalIsPending ()})
                                      else
                                        Transfer.Goto {args = Vector.new0 (), dst = continue}
                                    )
                                  end)
                            | Prim.Thread_atomicState => move (Runtime GCField.AtomicState)
                            | Prim.Thread_copy => simpleCCallWithGCState (CFunction.copyThread ())
                            | Prim.Thread_switchTo =>
                                ccall
                                  { args = (Vector.new3 (GCState, a 0, Operand.zero (WordSize.csize ())))
                            ...
                            | Prim.Weak_canGet =>
                                ifIsWeakPointer
                                  ( varType (arg 0)
                                  , fn _ => simpleCCallWithGCState (CFunction.weakCanGet {arg = Operand.ty (a 0)})
                            ...
                            | Prim.Weak_get =>
                                ifIsWeakPointer
                                  ( varType (arg 0)
                                  , fn t =>
                                      simpleCCallWithGCState (CFunction.weakGet {arg = Operand.ty (a 0), return = t})
                                  , fn () =>
                                      (case toRtype ty of
                                         NONE => none ()
                                       | SOME t => move (bogus t))
                            ...
                            | Prim.Weak_new =>
                                ifIsWeakPointer
                                  ...
                                  , fn t =>
                                      let
                                        val result = valOf (toRtype ty)
                                        val header = ObjptrTycon
                                          (case Type.deObjptr result of
                                             NONE => Error.bug "SsaToRssa.translateStatementsTransfer: PrimApp,Weak_new"
                                           | SOME opt => opt)
                                        val func = CFunction.weakNew {arg = t, return = result}
                                      in
                                        ccall
                                          { args = (Vector.concat [Vector.new2 (GCState, header), vos args])
                                          , func = func
                                          }
                                      end
                                  , none
                                  )
                            | Prim.Word_equal s => simpleCodegenOrC (Prim.Word_equal (WordSize.roundUpToPrim s))
                            | Prim.Word_toIntInf => cast ()
                            | Prim.Word_extdToWord (s1, s2, {signed}) =>
                                if WordSize.equals (s1, s2) then
                                  move (a 0)
                                else
                                  let
                                    val signed = signed andalso Bits.< (WordSize.bits s1, WordSize.bits s2)
                                    val s1 = WordSize.roundUpToPrim s1
                                    val s2 = WordSize.roundUpToPrim s2
                                  in
                                    if WordSize.equals (s1, s2) then cast ()
                                    else simpleCodegenOrC (Prim.Word_extdToWord (s1, s2, {signed = signed}))
                                  end
                            | Prim.WordVector_toIntInf => cast ()
                            | Prim.WordArray_subWord {eleSize, ...} => subWord eleSize
                            ...
                            | Prim.Region_push =>
                                if ! Control.optFuel <> NONE andalso Control.optFuelAvailAndUse () then
                                  ...
                                  let
                                    ...
                                    val oldBase = Var.newNoname ()
                                    val oldTop = Var.newNoname ()
                                    val newBase = Var.newNoname ()
                                    val oldBaseOp = Var {var = oldBase, ty = ptrTy}
                                    val oldTopOp = Var {var = oldTop, ty = ptrTy}
                                    val newBaseOp = Var {var = newBase, ty = ptrTy}
                                  ...
                                  in
                                    adds
                                      [ Bind {dst = (oldBase, ptrTy), pinned = false, src = Runtime GCField.RegionBase}
                                      , Bind {dst = (oldTop, ptrTy), pinned = false, src = Runtime GCField.RegionTop}
                                      , Move
                                          { dst = Offset {base = oldTopOp, offset = Bytes.zero, ty = ptrTy}
                                          , src = oldBaseOp
                                          }
                                      , PrimApp
                                          { args = Vector.new2 (oldTopOp, pointerSizeOperand)
                                          , dst = SOME (newBase, ptrTy)
                                          , prim = Prim.CPointer_add
                                          }
                                      , Move {dst = Runtime GCField.RegionBase, src = newBaseOp}
                                      , Move {dst = Runtime GCField.RegionTop, src = newBaseOp}
                                      ]
                                  end
                            | Prim.Region_pop =>
                                if ! Control.optFuel <> NONE andalso Control.optFuelAvailAndUse () then
      ...
      fun translateBlock (S.Block.T {label, args, statements, transfer}) =
        ...
        in
          Block.T {args = translateFormals args, kind = Kind.Jump, label = label, statements = ss, transfer = t}
        end
      fun translateFunction (f: S.Function.t) : Function.t =
        let
          val _ = S.Function.foreachVar (f, fn (x, t) => setVarInfo (x, {ty = t}))
          val {args, blocks, name, raises, returns, start, ...} = S.Function.dest f
          val _ = Vector.foreach (blocks, fn S.Block.T {label, args, ...} =>
            setLabelInfo (label, {args = args, cont = ref [], handler = ref NONE}))
          val blocks = Vector.map (blocks, translateBlock)
          val blocks = Vector.concat [Vector.fromList (!extraBlocks), blocks]
          val _ = extraBlocks := []
          fun transTypes (ts: S.Type.t vector option) : Type.t vector option =
            Option.map (ts, fn ts => Vector.keepAllMap (ts, toRtype))
        in
          Function.new
            { args = translateFormals args
            , blocks = blocks
            , name = name
      ...
      val main =
        ...
        in
          translateFunction (S.Function.profile
            ( S.Function.new
                { args = Vector.new0 ()
                , blocks = (Vector.new1 (S.Block.T
                    { label = start
                    , args = Vector.new0 ()
                    , statements = globals
                    , transfer = (S.Transfer.Call {args = Vector.new0 (), func = main, return = S.Return.Tail})
                    }))
                , mayInline = false
                , (* doesn't matter *)
